...
const writeAsAvailableFileTypes = (image: any, fileNameBase: string) => {
	FILETYPES.forEach((type: string) => image.write(OUTPUTFOLDER+fileNameBase+'.'+type)) 
}

Object.keys(ACTIONS).forEach((key: string) => {
	Jimp.read(ASSET).then((image: any) => {
